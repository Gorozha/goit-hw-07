...
//img вложенных в li.Для создания разметки используй шаблонные строки и
//insertAdjacentHTML().
...
const newTagUl = document.getElementById("gallery");

images.forEach(function (images) {
  const newTagLi = document.createElement("li");
  const newTagImg = document.createElement("img");
  newTagLi.prepend(newTagImg);
  newTagUl.prepend(newTagLi);
  newTagImg.prepend(images.url); //src
  newTagImg.insertAdjacentElement("afterbegin");
});
console.log(images);
